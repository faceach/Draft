(function() {
	...
	var C = -1 != z.indexOf("Opera") || -1 != z.indexOf("OPR"),
		D = -1 != z.indexOf("Trident") || -1 != z.indexOf("MSIE"),
		E = -1 != z.indexOf("Gecko") && -1 == z.toLowerCase().indexOf("webkit") && !(-1 != z.indexOf("Trident") || -1 != z.indexOf("MSIE")),
		F = -1 != z.toLowerCase().indexOf("webkit");
	(function() {
		var a = "",
			b;
		if (C && f.opera) return a = f.opera.version, "function" == k(a) ? a() : a;
		E ? b = /rv\:([^\);]+)(\)|;)/ : D ? b = /\b(?:MSIE|rv)[: ]([^\);]+)(\)|;)/ : F && (b = /WebKit\/(\S+)/);
		b && (a = (a = b.exec(z)) ? a[1] : "");
		return D && (b = (b = f.document) ? b.documentMode : void 0, b > parseFloat(a)) ? String(b) : a
	})();
	var G = function(a) {
			this.b = [];
			this.a = {};
	...
	function W(a) {
		return "42631044" == (H ? J(2) : "") || a && a.location && a.location.protocol && "https:" == a.location.protocol.toString().toLowerCase() ? "https:" : "http:"
	}
	...
	function aa(a, b, d, c) {
		...
		var e = W(a) + "//" + X(c) + "/pagead/" + [c.google_remarketing_only ? "viewthroughconversion/" : "conversion/", N(c.google_conversion_id), e, "random=", N(c.google_conversion_time)].join(""),
			g = d ? q(d) : "0",
			h;
		n: {
		...
		a = [P("cv", c.google_conversion_js_version), P("fst",
			...
			!c.google_conversion_domain ? "&srr=n" : "", P("vis", g)
		].join("");
		return e + a
	}
	...
	var ca = /Android ([01]\.|2\.[01])/i;

	function Y(a, b, d, c) {
		3 != c.google_conversion_format || c.google_remarketing_only || c.google_conversion_domain || H && I("317150500 317150501 317150502 317150503 317150504 317150505".split(" "), w, 1);
		var e = H ? J(1) : "";
		b = aa(a, b, d, c);
		d = function(a, b, c) {
			return '<img height="' + c + '" width="' + b + '" border="0" alt="" src="' + a + '" />'
		};
		return 0 == c.google_conversion_format && null == c.google_conversion_domain ? '<a href="' + (W(a) + "//services.google.com/sitestats/" + ba(c.google_conversion_language) + "?cid=" + N(c.google_conversion_id)) +
	...
	function fa(a) {
		...
		H = new G(1, 2, 3);
		"https:" == W(n) || ca.test(navigator.userAgent) || H && I(["42631043", "42631044"], x, 2);
		return !0
	}

	function ga(a) {
		for (var b = 0; b < L.length; b++) a[L[b]] = null
	}

	function ha(a) {
		for (var b = {}, d = 0; d < L.length; d++) b[L[d]] = a[L[d]];
		for (d = 0; d < M.length; d++) b[M[d]] = a[M[d]];
		return b
	}
	...
	(function(a, b, d) {
		if (a)
			if (null != /[\?&;]google_debug/.exec(document.URL)) ia(a);
			else {
				try {
					if (fa(a))
						if (H && I(["3145833456", "3145833457"], y, 3), "3145833457" == (H ? J(3) : "") && 3 == q(d)) {
							var c = ha(a),
