...
(function(DialogSPALib) {
    ...
    function setRequestData(cuInput) {
        try {
            ...
            DialogSPALib.logVerboseTrace("SPADialog: setRequestData", 0 /* Info */ , "cuInput:" + cuInput, "cuInput: " + cuInput + ",Json exception: " + err.message);
    ...
    function processIntermediateTurnInternal(ssml, postssmlAction, emotion, dictationParam) {
        DialogSPALib.logVerboseTrace("SPADialog: processIntermediateTurnInternal", 0 /* Info */ , "g_currentUIState: " + g_currentUIState + "; g_isSpeechInput: " + g_isSpeechInput + "; g_isDialogTerminated: " + g_isDialogTerminated, "postSsmlAction: " + postssmlAction + ", cuRequestHeaders" + JSON.stringify(g_cuRequestHeaders));
        if (g_spaDialogRuntime.changeSticStateAndInputMode) {
    ...
    function renderSPAUX(emotion) {
        DialogSPALib.logVerboseTrace("SPADialog: renderSPAUX", 0 /* Info */ , "emotion: " + emotion, "g_currentWebviewState: " + g_currentWebviewState);
    ...
    function processMicrophoneButtonPressed(dictationParam) {
        DialogSPALib.logVerboseTrace("SPADialog: processMicrophonebuttonpressed", 0 /* Info */ , "Entering processMicrophonebuttonpressed()", "speakinprogress :" + (g_currentUIState == 2 /* Speaking */ ) + ", g_isSpeechInput: " + g_isSpeechInput);
        if (!g_isSpeechInput) {
    ...
    function dialogTerminatedEventHandler() {
        DialogSPALib.logVerboseTrace("SPADialog: dialogTerminatedEventHandler", 0 /* Info */ , "g_currentSpeakOperation: " + g_currentSpeakOperation, "g_currentLuOperation: " + g_currentLuOperation);
    ...
    function processVoiceInput(dictationParameter) {
        DialogSPALib.logVerboseTrace("SPADialog: processVoiceInput", 0 /* Info */ , "cuRequestHeaders" + JSON.stringify(g_cuRequestHeaders), "cuInput :" + turn0CuInput);
        if (!turn0CuInput) {
            handoffDialogToCortana(2 /* Error */ );
            return;
        }
    ...
    function initializeTurn(inputQueryType, cuRequestHeaders) {
        ...
        t_currentImpressionId = createCUImpressionId();
        DialogSPALib.logVerboseTrace("SPADialog: inititalizeTurn", 0 /* Info */ , "IG:" + t_currentImpressionId, "");
    }
    ...
    function getTrumanProxyUrl() {
        return "/speech_render?speech=1&input=2&form=WNSBOX&cc=US&setlang=en-US";
    }
    ...
    function processResolutionActions(projectedApis, callBackUrl, taskFrameId, sessionId, locale, inputQueryType, cuRequestHeaders) {
        function evtAction() {
            try {
                ...
                initializeTurn(inputQueryType, cuRequestHeaders);
    ...
    function executeFinalAction(ssml, emotion, projectedApi, inputQueryType) {
        function evtAction() {
            try {
                sj_evt.unbind(spaDialogSjEvtName, evtAction);
    ...
    function processIntermediateTurn(ssml, postssmlAction, emotion, dictationParam, inputQueryType, cuRequestHeaders) {
        function evtAction() {
            try {
                sj_evt.unbind(spaDialogSjEvtName, evtAction);
                initializeTurn(inputQueryType, cuRequestHeaders);
    ...
    function processConfirmationAccept() {
        DialogSPALib.logVerboseTrace("SPADialog: processConfirmationAccept", 0 /* Info */ , "impression ID: " + t_currentImpressionId, "snr connection URL: " + trumanEndpoint);
        updateCortanaUIState(3 /* Thinking */ , 9 /* Thinking */ );
        var confirmUrl = "/search?input=1&q=yes";
        ClientResolutionRequested.CallEndPointGet(confirmUrl, t_currentImpressionId, g_cuRequestHeaders, function(innerHtml) {
    ...
    function processConfirmationReject() {
        DialogSPALib.logVerboseTrace("SPADialog: processConfirmationReject", 0 /* Info */ , "impression ID: " + t_currentImpressionId, "snr connection URL: " + trumanEndpoint);
        updateCortanaUIState(3 /* Thinking */ , 9 /* Thinking */ );
        var rejectUrl = "/search?input=1&q=no";
        ClientResolutionRequested.CallEndPointGet(rejectUrl, t_currentImpressionId, g_cuRequestHeaders, function(innerHtml) {
            ClientResolutionRequested.render(innerHtml);
        });
    }
    ...
    function processTimePickerData(hoursId, minutesId, secondsId) {
        DialogSPALib.logVerboseTrace("SPADialog: processTimePickerData", 0 /* Info */ , "impression ID: " + t_currentImpressionId, "");
        ...
        var timeUrl = "/?input=1&q=" + hours + minutes + seconds;
    ...
    function processHandoff() {
        DialogSPALib.logVerboseTrace("SPADialog: processHandoff", 0 /* Info */ , "impression ID: " + t_currentImpressionId, "snr connection URL: " + trumanEndpoint);
...
(function(ClientResolutionRequested) {
        ...
        var callback = function(response, isSuccess) {
            ...
            if (isSuccess) {
                ...
                if (index < projectedApis.length) {
                    ...
                    sendResolutionResultsToTCP(results, callBackUrl, taskFrameId, locale, sessionId, cuRequestHeaders);
                }
            } else {
    ...
    function sendResolutionResultsToTCP(clientResponse, callBackUrl, taskFrameId, locale, sessionId, cuRequestHeaders) {
        DialogSPALib.logVerboseTrace("ClientResolutionRequested: sendResolutionResultsToTCP", 0 /* Info */ , "clientResponse:" + clientResponse, "callbackUrl: " + callBackUrl + ", cuRequestHeaders: " + JSON.stringify(cuRequestHeaders));
        try {
            ajaxRequestWrapper(callBackUrl, function(url, onComplete) {
                callEndPoint(url, taskFrameId, locale, sessionId, false, clientResponse, cuRequestHeaders, onComplete);
            }, function(innerHtml) {
    ...
    function callEndPoint(url, taskFrameId, locale, sessionId, isSpeech, response, cuRequestHeaders, onSuccess) {
        DialogSPALib.logVerboseTrace("ClientResolutionRequested: callEndPoint", 0 /* Info */ , "url:" + url, "isSpeech: " + isSpeech);
